...
pipeline() {
    ...
    python3 edUKate/scripts/extract_textdocs.py --split-to-sents basic --doc-level "/DOC/ExercisePages" --empty-extext-as-special < ${fullpath} > ${tmpdir}/${file}.${srclang}.textdocs.jsonl
    ...
    echo "Stripping <ExText_*> tags from Textdocs content ${file}"
    sed 's|</\?ExText_[0-9]\+>||g' ${tmpdir}/${file}.${srclang}.textdocs.txt > ${tmpdir}/${file}.${srclang}.textdocs.no_extext.txt
    ...
    awk '{ print "<tu>" $0 "</tu>" }' ${tmpdir}/${file}.${srclang}.textdocs.no_extext.txt >> ${tmpdir}/${file}.${srclang}.textdocs.xml
    echo '</doc>' >> ${tmpdir}/${file}.${srclang}.textdocs.xml
    ...
    grep '<tu' ${tmpdir}/${file}.${trglang}.textdocs.xml | sed 's|<tu/>||g; s|<tu>||g; s|</tu>||g' > ${tmpdir}/${file}.${trglang}.textdocs.no_extext.txt

    echo "Fixing common XML issues in the translated Textdocs content ${file}"
    python3 fix_textdocs_structure.py ${tmpdir}/${file}.${trglang}.textdocs.no_extext.txt ${tmpdir}/${file}.${trglang}.textdocs.no_extext.fixed.txt

    echo "Inserting ExText elements into the translated Textdocs content ${file}"
    python3 insert_extexts.py --reference ${tmpdir}/${file}.${srclang}.textdocs.txt ${tmpdir}/${file}.${trglang}.textdocs.no_extext.fixed.txt ${tmpdir}/${file}.${trglang}.textdocs.txt

    echo "Reconstructing the Textdocs JSON structure using the translated content ${file}"
    python3 replace_textdocs_texts.py ${tmpdir}/${file}.${trglang}.textdocs.txt < ${tmpdir}/${file}.${srclang}.textdocs.jsonl > ${tmpdir}/${file}.${trglang}.textdocs.jsonl

    echo "Reconstructing the original XML structure using the translated Textdocs ${file}"
    python3 edUKate/scripts/import_textdoc_to_orig.py --empty-extext-as-special ${tmpdir}/${file}.${trglang}.textdocs.jsonl < ${fullpath} > ${outdir}/${file}
}
